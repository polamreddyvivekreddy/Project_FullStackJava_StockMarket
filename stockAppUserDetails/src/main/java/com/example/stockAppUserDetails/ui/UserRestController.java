...
public class UserRestController {
	...
	@GetMapping("/{pattern}")
	public List<UserDto> findByUserNameContaining(@PathVariable("pattern") String pattern)
			throws UserNotFoundException {
		List<UserEntity> userEntity = userService.findByUserNameContaining(pattern);
		ModelMapper mapper = new ModelMapper();
		Type listType = new TypeToken<List<UserDto>>(){}.getType();
        List<UserDto> postDtoList = mapper.map(userEntity,listType);
        return postDtoList;
	}
